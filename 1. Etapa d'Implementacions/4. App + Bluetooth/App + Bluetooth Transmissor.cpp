...
#include <SoftwareSerial.h> // Per al HC-06
...
int Coma1BLE, Coma2BLE, Coma3BLE; // Variables per separar els valors rebuts

/****  Interruptor HC-06  ****/
#define interruptor 4 // Si aquest és definit (HC-06 Habilitat), el monitorSerie no ha d'estar definit o no serà possible una bona comunicació
...
void setup() {
  /****  Monitor Sèrie o HC-06 ****/
  // Si només una de les variables és definida inicialitzo el monitor sèrie
  #if (defined(monitorSerie) && !defined(interruptor)) || (!defined(monitorSerie) && defined(interruptor))
...
void loop() {
  // Depenent de si l'interruptor està encès s'activarà el control amb joysticks o amb l'aplicació
  if(!digitalRead(interruptor)) {
    ...
    dadesIEnviar();
  } else if(digitalRead(interruptor)) { 
    /****  HC-06  ****/
    if(Serial.available()) {
      caracter = Serial.read();
      dataBLE = dataBLE + caracter;

      if(caracter == '*') { // El * és un delimiter, separa la informació en les quatre velocitats juntes
        dataBLE = dataBLE.substring(0, dataBLE.length() -1); // Elimino el caràcter delimiter de les quatre velocitats
    
        Coma1BLE = dataBLE.indexOf(','); // Obtinc la posició en la informació de la primera coma
        throttle = dataBLE.substring(0, Coma1BLE).toInt();
        Coma2BLE = dataBLE.indexOf(',', Coma1BLE + 1);
        yaw = dataBLE.substring(Coma1BLE + 1, Coma2BLE).toInt();
        Coma3BLE = dataBLE.indexOf(',', Coma2BLE + 1);
        pitch = dataBLE.substring(Coma2BLE + 1, Coma3BLE).toInt();
        //ind4 = dataBLE.indexOf(',', ind3+1);
        roll = dataBLE.substring(Coma3BLE + 1).toInt();

        dataBLE = "";

        // Estableixo els límits de cada variable perquè la connexió Bluetooth no és molt estable
        if(throttle > 1887) {
          throttle = 1887;
      ...
      dadesIEnviar(); // Cal executar el codi en el moment que es rep la informació, però igualment vaig tenir problemes de sincronització i realitat
    }
  }
}