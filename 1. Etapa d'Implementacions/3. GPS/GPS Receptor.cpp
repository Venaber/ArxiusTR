...
#include <SoftwareSerial.h> // Per al GPS
#include <TinyGPSPlus.h> // Per al GPS
...
/****  GPS  ****/
SoftwareSerial comunicGPS(4, 5); // RX i TX Nano
TinyGPSPlus dataGPS;


/****  BMP280 + GPS  ****/
float ack[3]; // Array per emmagatzemar les dades a enviar
...
void setup() {
  ...
  /****  GPS  ****/
  comunicGPS.begin(9600); // Inicialitzo el mòdul GPS
...
void loop() {
  ...
  if (radio.available()) {
    ...
    /****  BMP280 + GPS ****/
    // Creo un packet de 3 dades amb l'alçada (BMP280) i coordenades (GPS)
    ack[0] = bmp.readAltitude(1013.25) - puntInicial;
    
    while (comunicGPS.available()) { // El principal problema és el while i que el mòdul GPS envia informació cada 1 segons automàticament
      if (dataGPS.encode(comunicGPS.read())) {
        if (dataGPS.location.isValid()) {
          #ifdef monitorSerie
            Serial.print("Latitu: ");
            Serial.println(dataGPS.location.lat(), 6);
            Serial.print("Longitd: ");
            Serial.println(dataGPS.location.lng(), 6);
          #endif
          
          ack[1] = dataGPS.location.lat();
          ack[2] = dataGPS.location.lng();
        } else { // Si el mòdul no està operatiu mostro uns valors determinats per senyalitzar
          ack[1] = 20.700001;
          ack[2] = 20.700001;
        }
      }
