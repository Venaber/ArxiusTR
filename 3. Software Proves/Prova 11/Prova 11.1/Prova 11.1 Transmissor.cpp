...
void loop() {
  ...
  int valorYMapped = map(valorY, 0, 1023, 0, 255); // Comença en un rang 0 - 1023 perquè és un valor analògic i cal que sigui a un rang de 0-255 perquè en el receptor utilitzaré un senyal PWM
  ...
  const int radioMessage = valorYMapped; // Declaro el missatge a enviar
  radio.write(&radioMessage, sizeof(radioMessage)); // Envio el missatge
}